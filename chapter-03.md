...
这与区块链需要解决的问题非常相似，都是为了链下DOS实例在开放的拜占庭网络环境中就相同API调用的结果达成一致。来回顾下有关比特币与其他各种共识算法是如何工作的：实质上他们尝试在每一轮协议中，希望通过没有任何一方能够轻易垄断节点资源，去实现随机的领导者选举；只要大多数网络是可信的，区块链就会在概率意义上达成共识。生成公平、无偏倚的随机数对完成拜占庭容错（BFT）共识机制中尤为重要。下面我们将主要利用可验证随机函数(VRF)和门限签名方案[25]演示链下共识协议。简单来说，我们不是在所有节点中进行领导者选举，而是在每个协议运行的所有注册组之间进行随机分组选择；利用门限签名加密，只要超过 t（阈值）成员是诚实的，所选择的组就可以达到链下共识。
